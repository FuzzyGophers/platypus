...
/// `FTTone` value-carrier display / mode-label (pure). Tone *parsing* (source string → tone) now
/// lives in the core (`program_tone`) and is tested there.
final class FTToneTests: XCTestCase {
    func testDisplayAndModeLabel() {
        XCTAssertEqual(FTTone.ctcss(156.7).display, "CTCSS 156.7 Hz")
        XCTAssertEqual(FTTone.ctcss(100.0).modeLabel, "CTCSS")
        XCTAssertEqual(FTTone.dcs(23).display, "DCS 023")
        XCTAssertEqual(FTTone.dcs(23).modeLabel, "DCS")
...
/// `FT60Memory` bank/slot/capacity logic.
final class FT60MemoryTests: XCTestCase {
    ...
    // Catalog → FT-60 channel mapping (mode/tone/offset) now lives in the core (`from_program`,
    // exercised by platypus-core's ft60 synth tests) and is exposed via
    // `platypus_ft60_channel_from_catalog`; the app no longer has a `makeFromCatalog` to unit-test.
