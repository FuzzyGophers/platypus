...
//! input. As a hard backstop, a panic must never unwind across this boundary: `extern "C"`
//! aborts on unwind (Rust ≥ 1.81), and the workspace sets `panic = "abort"` for release
//! (`Cargo.toml`) to make that guarantee explicit and toolchain-independent. Graceful
//! per-call `catch_unwind` recovery is a possible future refinement (see `TODO.md`).
...
pub extern "C" fn platypus_ft60_options_json() -> *mut c_char {
    ...
    let emit =
        |out: &mut String, key: &str, list: &[platypus_core::device::FieldOption], tone: bool| {
            out.push('"');
            out.push_str(key);
            out.push_str("\":[");
...
pub unsafe extern "C" fn platypus_library_catalog_json(
    ...
    let filter = Filter::from_c(services_csv, techs_csv, hide_encrypted, search);
...
pub unsafe extern "C" fn platypus_library_channels_json(
    ...
    let filter = Filter::from_c(services_csv, ptr::null(), hide_encrypted, search);
    ...
    for line in sys.lines() {
        ...
        if matches!(cmd, "C-Group" | "T-Group") {
            dept_in_range = Record::new(line, lib.profile)
                .and_then(|r| r.geo())
                .is_some_and(|g| {
                    g.lat != 0.0 && g.lon != 0.0 && haversine_miles(lat, lon, g.lat, g.lon) <= miles
                });
        } else if is_voice_channel(cmd) {
            let this = ci;
            ci += 1;
...
fn push_catalog_system(out: &mut String, si: usize, sys: &System, profile: &dyn SdCardProfile) {
    ...
    for line in sys.lines() {
        ...
        // Encryption isn't decoded from the card yet (RR exposes an `enc` flag we
        // don't carry through the HPDB model). Emit false so the UI affordance
        // exists; wire real detection when the attribute lands. TODO.
        out.push_str(",\"encrypted\":false}");
        ci += 1;
    }
    out.push_str("]}");
}
...
fn count_channels(sys: &System, profile: &dyn SdCardProfile, filter: &Filter) -> usize {
    ...
    for line in sys.lines() {
        ...
        // Encryption isn't decoded yet (see catalog_json); treat as false.
        if filter.channel_ok(
...
pub unsafe extern "C" fn platypus_ft60_read(
    ...
    let set_err = |msg: String| {
        if !err_out.is_null() {
            unsafe { *err_out = to_c_string(msg) };
        }
    };
...
pub unsafe extern "C" fn platypus_ft60_memories_json(handle: *const PlatypusFt60) -> *mut c_char {
    ...
    for (i, ch) in h.image.channels().iter().enumerate() {
        ...
        let (tmode, tval) = match ch.tone {
            Tone::None => ("off", 0i64),
            Tone::Ctcss(hz10) => ("ctcss", hz10 as i64),
            Tone::Dcs(code) => ("dcs", code as i64),
        };
        // Tone-mode sub-kind as its TMODES index, plus the value-kind + value for the squelch.
...
pub unsafe extern "C" fn platypus_ft60_apply(
    ...
    for c in list {
        ...
        // tone_mode is the TMODES index (0-7); 1–3 use the CTCSS value, 4–7 the DCS value.
...
mod tests {
    ...
    fn library_aggregate_load_filter_and_lazy_channels() {
        unsafe {
            ...
            let cj = platypus_library_catalog_json(
                lib,
                empty.as_ptr(),
                empty.as_ptr(),
                false,
            ...
            let chj = platypus_library_channels_json(
                lib,
                id.as_ptr(),
                empty.as_ptr(),
                false,
            ...
            let fj = platypus_library_catalog_json(
                lib,
                empty.as_ptr(),
                p25.as_ptr(),
                false,
    ...
    fn favorites_edit_open_remove_append_commit() {
        unsafe {
            ...
            let cat =
                platypus_library_catalog_json(lib, all.as_ptr(), all.as_ptr(), false, all.as_ptr());
